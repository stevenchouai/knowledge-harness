...
DEMO_BUNDLE_FILES = (
    ...
    Path("prompt.txt"),
    Path("run.json"),
    Path("fake-vault") / "AGENTS.md",
    Path("fake-vault") / "PROMPTS.md",
    Path("fake-vault") / "wiki" / "_index.md",
)
DEMO_BUNDLE_DIRS = {
    Path("fake-vault"),
    Path("fake-vault") / "wiki",
}
...
def build_demo_command(
    ...
    *,
    ...
    save_bundle: Path | str | None = None,
) -> str:
    ...
    if save_bundle is not None:
        command.extend(["--save-bundle", str(save_bundle)])
...
def build_demo_bundle_readme(receipt: dict[str, object]) -> str:
    return "\n".join(
        [
            "# knowledge-harness demo evidence bundle",
            "",
            "This directory is a public-safe, self-contained proof bundle generated by "
            "`knowledge-harness demo --save-bundle PATH`.",
            "",
            "## Demo",
            "",
            ...
            "## Files",
            "",
            "- `fake-vault/AGENTS.md` is the demo-only behavior contract.",
            "- `fake-vault/PROMPTS.md` is the demo-only prompt contract.",
            "- `fake-vault/wiki/_index.md` is the demo-only routing index.",
            "- `prompt.txt` is the assembled dry-run prompt.",
            "- `run.json` is the dry-run metadata.",
            "- `receipt.md` is the Markdown proof receipt.",
            "",
            "## Verification checklist",
            "",
            "1. Open `fake-vault/AGENTS.md`, `fake-vault/PROMPTS.md`, and "
            "`fake-vault/wiki/_index.md`; confirm they are fake demo files only.",
            "2. Open `prompt.txt`; confirm it contains the fake vault contracts and the "
            "demo question.",
            "3. Open `run.json`; confirm `dry_run` is `true`, `write_output` is `false`, "
            "and `question` matches the README.",
            "4. In `run.json`, confirm `command` uses `missing-codex` and does not include "
            "`--add-dir` or any real vault path.",
            "5. Confirm there is no `last_message.txt`; a dry run prepares evidence but does "
            "not invoke Codex.",
            "6. Open `receipt.md`; confirm the safety claims are `real_vault_used=false`, "
            "`codex_used=false`, `write_output=false`, and `dry_run=true`.",
            "",
            "No real Obsidian vault, Codex credentials, tokens, or private vault content are "
            "needed to inspect this bundle.",
            "",
        ]
    )


def bundle_relative_demo_receipt(question: str) -> dict[str, object]:
    return {
        "question": question,
        "command": build_demo_command(question, save_bundle="PATH"),
        ...
        "dry_run": True,
        "cleanup_root": ".",
        "cleanup_command": "remove this bundle directory when finished",
    }


def check_demo_bundle_target(bundle_path: Path) -> None:
    if bundle_path.exists() and not bundle_path.is_dir():
        raise SystemExit(f"save_bundle path must be a directory, not a file: {bundle_path}")

    if bundle_path.exists():
        allowed_files = set(DEMO_BUNDLE_FILES)
        allowed_dirs = set(DEMO_BUNDLE_DIRS)
        unexpected: list[str] = []
        for child in sorted(bundle_path.rglob("*")):
            relative = child.relative_to(bundle_path)
            if child.is_symlink():
                unexpected.append(relative.as_posix())
            elif child.is_dir():
                if relative not in allowed_dirs:
                    unexpected.append(f"{relative.as_posix()}/")
            elif child.is_file():
        ...
        if unexpected:
            ...
            raise SystemExit(
                "save_bundle target exists and contains files this command does not own:\n"
                f"{unexpected_lines}"
            )


def prepare_demo_bundle_target(bundle_path: Path) -> None:
    check_demo_bundle_target(bundle_path)
    if not bundle_path.exists():
        bundle_path.parent.mkdir(parents=True, exist_ok=True)
        bundle_path.mkdir()


def demo_bundle_metadata(run_path: Path) -> dict[str, object]:
    metadata = json.loads((run_path / "run.json").read_text(encoding="utf-8"))
    raw_command = metadata.get("command", [])
    if isinstance(raw_command, list):
        sanitized_command: list[object] = []
        skip_next_output_path = False
        skip_next_cd_path = False
        for part in raw_command:
            if skip_next_output_path:
                sanitized_command.append("last_message.txt")
                skip_next_output_path = False
                continue
            if skip_next_cd_path:
            ...
            if part == "--output-last-message":
                ...
                skip_next_output_path = True
            elif part == "--cd":
                sanitized_command.append(part)
                skip_next_cd_path = True
            elif isinstance(part, str) and part.endswith("missing-codex"):
                sanitized_command.append("missing-codex")
            else:
                sanitized_command.append(part)
        metadata["command"] = sanitized_command
    return metadata
...
def write_demo_bundle(
    ...
    prepare_demo_bundle_target(bundle_path)

    (bundle_path / "fake-vault" / "wiki").mkdir(parents=True, exist_ok=True)
    shutil.copyfile(run_path / "prompt.txt", bundle_path / "prompt.txt")
    (bundle_path / "run.json").write_text(
        json.dumps(demo_bundle_metadata(run_path), ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
    shutil.copyfile(fake_vault / "AGENTS.md", bundle_path / "fake-vault" / "AGENTS.md")
    shutil.copyfile(fake_vault / "PROMPTS.md", bundle_path / "fake-vault" / "PROMPTS.md")
    shutil.copyfile(
        fake_vault / "wiki" / "_index.md",
        bundle_path / "fake-vault" / "wiki" / "_index.md",
    )

    receipt = bundle_relative_demo_receipt(question)
    (bundle_path / "receipt.md").write_text(
        format_demo_markdown_receipt(receipt),
        encoding="utf-8",
    )
    (bundle_path / "README.md").write_text(
        build_demo_bundle_readme(receipt),
        encoding="utf-8",
    )
    return receipt
...
def run_demo(
    ...
    *,
    ...
    save_bundle = validate_demo_bundle_path(save_bundle)
    if save_bundle is not None:
        check_demo_bundle_target(save_bundle)
    ...
    if save_markdown is not None:
        ...
        return 0
    if save_bundle is not None:
        receipt = write_demo_bundle(save_bundle, fake_vault, run_path, question)
        print("knowledge-harness demo")
        print(f"question: {receipt['question']}")
        print(f"saved_bundle: {save_bundle}")
        print(f"readme: {save_bundle / 'README.md'}")
        print(f"status: {receipt['status']}")
        print(f"cleanup: rm -rf {shlex.quote(str(demo_root))}")
        return 0
...
def validate_demo_bundle_path(path: Path | None) -> Path | None:
    if path is None:
        return None
    ...
    expanded = path.expanduser()
    if raw_path in {"", ".."}:
        raise SystemExit("save_bundle path must be a directory path")
    if ".." in expanded.parts:
        raise SystemExit("save_bundle path must not contain parent traversal")
    if "\\" in raw_path:
        raise SystemExit("save_bundle path must use forward slashes")
    if expanded.exists() and not expanded.is_dir():
        raise SystemExit(f"save_bundle path must be a directory, not a file: {expanded}")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    demo_output.add_argument(
        ...
        dest="save_bundle",
        metavar="PATH",
        help="Write a self-contained public-safe evidence bundle under PATH.",
    )
